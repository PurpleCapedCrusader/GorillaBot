...
bot.on("message", (message) => {
	...
	switch (args[0]) {
		...
		//                                         ) {
		//                                             // startGame(message);
		//                                             // play((message) => startGame(message));
		//                                             startGame(message).then(
		//                                                 () => {
		//                                                     play(message);
		//                                                 }
		...
		//                 );
		//                 console.log(
		//                     `START FAILED: Not using a game room text channel`
		//                 );
		//             }
		//         } else {
		//             message.channel.send(
...
async function leave(message) {
	// TODO: allow players to remove themselves from the game
	const client = await pool.connect();
	try {
		getGameId(message).then((results) => {
			gameId = results;
		...
		console.log(
			`leave() > parseInt(gameId.rows) = ${parseInt(gameId.rows)}`
		);
		console.log(`leave() author_id = ${message.author.id}`);
...
async function roll_for_game(message) {
	...
	try {
		...
		playersFromDatabase.rows.forEach((row) => {
			...
			playersFromDatabaseArray.push(message.guild.member(playersInGame));
		...
		playersFromDatabaseArray.forEach(function (guildMember, guildMemberId) {
			console.log(
				`JSON.stringify(guildMember) = ${JSON.stringify(guildMember)}`
			);
...
async function startGame(message) {
	...
	try {
		...
		const insertGameStartText =
			...
			`voice_channel_id, message_id, game_theme, theme_category_roll_array, ` +
